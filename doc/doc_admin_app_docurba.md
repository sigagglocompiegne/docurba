...
|  affiche_avap_result_parcelle |x|x||Déclaré en HTML. Formatage affichage du résultat d'une parcelle|Fiche de renseignements d'urbanisme|`'<b>' || {CCOSEC} || {BG_NUMPARC} || '</b> ' || upper({commune}) || '<br>' || CASE WHEN {BG_PROP_RECORDS} is null OR {BG_PROP_RECORDS} = '' THEN 'Fiche propriétaire inconnue ou non renseignée.' ELSE '<font size=3>Adresse (fiscale) : ' || {BG_FULL_ADDRESS} END || '</font><br><br>'`|
